...
sys.path.append('/home/mongardi/Synth_data/src')
...
def detection_pca(data_real, data_gen, data_real_test, data_fake_test):
    #data_real_test, data_fake_test, cancer_label_test, tissue_label_test= self.generate_samples(real_test)
...
def Classifiers(X_train, y_train, X_test, y_test,  description , result_dire =None, name= None, save = True, detection = False, cm = False):
        models = {
        ...
        for model_name, model in models.items():
            ...
            #print(f"PIETRO - {model_name} : {[(i, len(list(filter(lambda x : x==i, y_pred)))) for i in range(4)]}")
            ...
            y_pred, y_test, labels, name, result_dire, save=save, description=description, detection = detection 
            )
            ...
            if cm == True: 
                ...
                plt.title(f'Training {model_name}')
                plt.xlabel('Predicted Labels')
                plt.ylabel('True Labels')
            
                filename = f'wpgan_confusion_matrix_{model_name}.png'
      
                plt.savefig(os.path.join(result_dire, filename))
                plt.show()
        return results     

def tissues_classification(data_real,tissue_label_real, data_gen ,tissue_label,cm = False,results_dire=None):
 
            
    
        cl5_r_vs_f = Classifiers(data_real, tissue_label_real , data_gen, tissue_label, None, result_dire= results_dire, save = False, detection = False, cm=cm)
