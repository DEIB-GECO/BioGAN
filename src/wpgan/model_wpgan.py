...
class WGAN_GP():
    ...
    def build_WGAN_GP(self):
        ...
        # to do: fix bug 
        self.numerical_dims = []
        self.gen, self.disc = WGAN_GP_model(self.latent_dims, self.input_dims, self.numerical_dims, 
                self.vocab_sizes, 
                self.generator_dims, 
                self.discriminator_dims, self.stat1, self.stat2,
    ...
    def generate_samples_all(self, data):
        ...
        for i, data in enumerate(data):
            ...
            num_categories = 3  # Categorie: 0, 1, 2, 3
            elements_per_category = num_elements // num_categories

        # Crea un vettore bilanciato con elementi proporzionali
            balanced_x_cat = torch.tensor(
            np.random.permutation(
            np.tile(np.arange(1, num_categories+1 ), elements_per_category)  # Shift to 1, 2, 3
            )
            ).to(self.device).unsqueeze(1).to(self.device)
            #balanced_x_cat_array.append(balanced_x_cat[0].cpu().numpy())
            balanced_x_cat_array.extend(balanced_x_cat.cpu().numpy())    
                 
            #print("balanced_x_cat", len(balanced_x_cat_array))
            #print("x_GE", x_GE.shape)
            #x_real, x_gen = self.generate_samples(x_GE, x_cat)
            x_real, x_gen = self.generate_samples(x_GE, balanced_x_cat)
            if i==0:
        ...
        return all_real_x, all_real_gen, balanced_x_cat_array,all_tissue#all_tissue
    ...
    def test(self, test_data, compute_score=True):
        ...
        for i, data in enumerate(test_data):
            ...
            #balanced_x_cat_array.extend(balanced_x_cat.cpu().numpy()) 
            #balanced_x_cat_array = balanced_x_cat_array.cpu().numpy()
        ...
        return all_real, all_gen, balanced_x_cat_array, all_tissue
    ...
    def fit(self, train_data, val_data, test_data, epochs, val=True):
        ...
        for epoch in range(epochs):
            ...
            #wandb.log({"discriminator loss": d_loss_all/len(train_data)})
            ...
            if val:       
                ...
                if (epoch+1) % self.freq_compute_test == 0:
                    ...
                    # if early_stopper.early_stop(self.results_dire, self.gen, self.disc, self.corr_scores[epoch+1], epoch+1):
                        # pass 
                    ...
                    #metrics = compute_evaluation_metrics(data_real, data_gen, all_real, all_gen)
                        ...
                        for run in range(n_runs):
                            ...
                            print(np.concatenate((data_real, all_real), axis=0).shape)
                            print(pd.Series(np.concatenate((data_real, all_real), axis=0)[:,0]).describe())
                            ...
                            results_utility_TSTR = tissues_classification(all_gen, all_tissue_test ,all_real, all_tissue, cm=True, results_dire=results_dire_run)
